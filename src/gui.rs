...
unsafe extern "system" fn dlg_proc(dialog: HWND, message: u32, wparam: WPARAM, lparam: LPARAM) -> isize {
    match message {
        WM_INITDIALOG => {
            ...
            if let Ok(icon) = LoadIconW(instance, IDI_SHINMY) {
            ...
            _ = SetWindowTextW(
                packaged_ver_static,
                &HSTRING::from(format!("Packaged version: {}", env!("SHINMY_VERSION")))
            );
            ...
            // Defaults to already installed Shinmy dll, if any
            ...
            if multiple_installs {
                MessageBoxW(
                    dialog,
                    w!("Multiple installations of Shinmy detected! \
        ...
        WM_COMMAND => {
            ...
            match control_id {
                ...
                IDC_INSTALL => {
                    let installer = get_installer(dialog);
                    if let Some(target) = installer.get_shinmy_installed_target() {
                        if target != installer.target {
                            MessageBoxW(
                                dialog,
                                &HSTRING::from(format!("Shinmy is already installed as {}", target.dll_name())),
                                w!("Error"),
                                MB_ICONERROR | MB_OK
                            );
                            return 0;
                ...
                IDC_UNINSTALL => {
                    ...
                    if res == IDOK {
                        ...
                        if let Some(version_info) = version_info_opt {
                            ...
                            // Check if the shinmy data dir exists and prompt user to delete it
                            let shinmy_dir = installer.install_dir.as_ref().unwrap().join("shinmy");
                            let Ok(metadata) = std::fs::metadata(&shinmy_dir) else {
                                return 0;
                            };
                            ...
                            if metadata.is_dir() {
                                let res = MessageBoxW(
                                    dialog,
                                    w!("Do you also want to delete Shinmy's data directory? \
                                        The game may crash if it is present without Shinmy.\n\
                                ...
                                if res == IDYES {
                                    if let Err(e) = std::fs::remove_dir_all(&shinmy_dir) {
                                        MessageBoxW(dialog, &HSTRING::from(e.to_string()), w!("Error"), MB_ICONERROR | MB_OK);
                                        return 0;
                                    }
                                }
